...
def rJnz(cod, p):
    return (cod[p], p + 1)[ds.pop()]


def rJz(cod, p):
    return (p + 1, cod[p])[ds.pop() == 0]
...
def rCreate(pcode, p):
    ...
    # match next word (input) to next heap address
    lastCreate = label = getWord()
...
def rAt(cod, p):
    ds.append(heap[ds.pop()])       # get heap @ address


def rBang(cod, p):
    a = ds.pop()
    heap[a] = ds.pop()  # set heap @ address
    ...
    while 1:
        ...
        if cAct:
            ...
            if type(rAct) == type([]):
                ...
                pcode.append(rAct)  # push builtin for runtime
        else:
...
def cColon(pcode):
    if cStack:
        fatal(": inside Control stack: %s" % cStack)
    label = getWord()
...
def cUntil(pcode):
    ...
    if code != "BEGIN":
        fatal("UNTIL preceded by %s (not BEGIN)" % code)
    pcode.append(rJz)
    pcode.append(slot)
...
def cElse(pcode):
    ...
    code, slot = cStack.pop()
    if code != "IF":
        fatal("ELSE preceded by %s (not IF)" % code)
    pcode.append(rJmp)
...
def cThen(pcode):
    ...
    if code not in ("IF", "ELSE"):
        fatal("THEN preceded by %s (not IF or ELSE)" % code)
